...
//     on<AuthStarted>((_, emit) {
//       emit(AuthLoading());
//       _sub = repo.stream.listen((u) {
//         if (u == null) add(AuthLoggedOut()); else add(AuthLoggedIn());
//       });
//     });
//     on<AuthLoggedOut>((_, emit) => emit(AuthUnauthenticated()));
//     on<AuthLoggedIn>((_, emit) {
//       final u = repo.current!;
//       saveFcmTokenForCurrentUser(); // store FCM token
//       emit(AuthAuthenticated(u));
//     });
//     on<AuthSubmit>((e, emit) async {
//       emit(AuthLoading());
//       try {
//         if (e.login) { await repo.signIn(e.email, e.password); }
//         else { await repo.signUp(e.email, e.password); }
//       } catch (err) { emit(AuthError(err.toString())); emit(AuthUnauthenticated(loginMode: e.login)); }
//     });
//   }
//
//   @override
